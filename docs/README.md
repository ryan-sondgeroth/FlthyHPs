...
## Understanding the Sketch Settings
Explanation of User Definable Settings in the v2.1 Code (v1.8-compatible with v2.1 additions)
...
```cpp
constexpr uint8_t I2C_ADDRESS = 0x19;  // 25 in hexadecimal
```
...
```cpp
constexpr uint8_t SERVO_I2C_ADDRESS = 0x40;  // 64 in hexadecimal
```
...
const uint8_t HP_PINS[Config::HP_COUNT][2] PROGMEM = {{0,1}, {2,3}, {4,5}};
```
...
```cpp
constexpr uint8_t RC_PIN = 9;  // active only when ENABLERC is enabled
```
...
```cpp
const uint16_t RC_RANGE[2] PROGMEM = {1250, 1750};     // {Min, Max}
```
...
```cpp
constexpr uint8_t DIM_PULSE_SPEED = 5;  // Default speed if no value is supplied
const uint8_t DIM_PULSE_SPEED_RANGE[2] PROGMEM = {5, 75};  // Lower values are faster.
```
...
Value Options:   F - Front HP, X - Front & Rear HPs
R - Rear HP, Y - Front & Top HPs
T - Top HP, Z - Rear & Top HPs
A - All 3 HPs, S - Special Sequence Commands
T - the Sequence Type;
...
0971 (D0971) - Clears LED, disables “Off Color”, Enables Auto LED Sequence using Default Sequences*
0972 (D0972) - Clears LED, disables “Off Color”, Enables Auto LED Sequence using Random Sequences*
098  (D098)  - Clears LED, enables “Off Color”, Disables Auto LED Sequence*
0991 (D0991) - Clears LED, enables “Off Color”, Enables Auto LED Sequence using Default Sequences*
0992 (D0992) - Clears LED, enables “Off Color”, Enables Auto LED Sequence using Random Sequences*
...
“R0053”   - Rear HP LEDs set solid Green (R/0/05/3)
“F00369” - Front HP Dim Pulse in Magenta at speed 9 (F/0/03/6/9)
“T006”     - Starts Rainbow sequence on the Top HP LEDs. (T/0/06)
“A0971”   - Clears all HP LEDs and enables default auto LED twitch. (A/0/97/1)
“F1012”   - Sends the Front HP to preset position 2 (Up). (F/1/01/2)
“A1011”   - Sends all three HPs to preset position 1 (Center). (A/1/01/1)
“R103”     - Enables RC control (Up/Down axis) on Rear HP. (R/1/03)
“T199”     - Enables Servo auto twitch mode for Top HP. (T/1/99)
...
I2C communication does not require this extra character. Type “help” or “?” in Serial Monitor for built-in command help.
...
A006|45
```
…would run the Rainbow Sequence on all 3 HPs for 45 seconds, then clear and return to the prior auto state.
